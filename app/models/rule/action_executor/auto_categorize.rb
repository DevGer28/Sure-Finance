class Rule::ActionExecutor::AutoCategorize < Rule::ActionExecutor
  def label
    base_label = I18n.t("rules.action_executors.labels.auto_categorize")
      ...
      if llm_provider
        ...
        suffix =
          if estimated_cost.nil?
            I18n.t("rules.action_executors.auto_categorize.cost_na_suffix")
          else
            I18n.t(
              "rules.action_executors.auto_categorize.cost_estimate_suffix",
              amount: sprintf("%.4f", estimated_cost)
            )
          end
        "#{base_label}#{suffix}"
      else
        "#{base_label}#{I18n.t('rules.action_executors.auto_categorize.no_provider_suffix')}"
      end
    else
      base_label
    end
